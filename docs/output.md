...
This step filters each ortholog FASTA file to retain only IDs selected by the final filtering logic, then creates a downstream samplesheet for nf-core/multiplesequencealign. The `fasta` column stores absolute paths to the filtered FASTA files in the pipeline output directory.
